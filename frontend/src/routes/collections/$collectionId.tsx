...
function renderAlphabetCell(alphabet: string) {
  switch (alphabet) {
    case "dna":
      return <Chip label="DNA" color="primary" />;
    case "rna":
      return <Chip label="RNA" color="error" />;
    case "protein":
      return <Chip label="Protein" color="info" />;
  }
}
...
function CollectionDetailsPage() {
  ...
  const { isLoading, data } = useQuery({
    ...
    queryFn: () =>
      apiClient.getCollectionSequences(
        collection.id,
    ...
    <Box sx={{ width: "100%", p: 2 }}>
      <Box sx={{ mb: 4 }}>
        <Typography
          variant="h3"
          component="h1"
          sx={{ fontWeight: 700, color: "primary.main", wordBreak: "break-word" }}
        >
          {collection.name}
        </Typography>
        ...
        <Typography variant="caption" color="text.secondary" sx={{ display: "block", mt: 1 }}>
          Created {new Date(collection.created_at as unknown as string).toLocaleString()} • Last
          modified {new Date(collection.last_modified as unknown as string).toLocaleString()}
        </Typography>
      </Box>

      <Card>
        <CardHeader
          title="Sequences in this collection"
                  ...
                  <TableRow>
                    <TableCell colSpan={7} align="center">
                      <Typography variant="body2" color="text.secondary">
                        No sequences in this collection
                      </Typography>
                    </TableCell>
                  </TableRow>
                ) : (
                    ...
                    <TableRow
                      ...
                      onClick={() =>
                        navigate({
                          ...
                          params: { sequenceId: row.id.toString() },
                        })
                      }
                      sx={{ cursor: "pointer" }}
                    >
                      ...
                      <TableCell>
                        {new Date(row.created_at as unknown as string).toLocaleString()}
                      </TableCell>
